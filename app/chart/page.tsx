...
function useCurrentPrice(symbol: string, exchange: string) {
  ...
  useEffect(() => {
    ...
    if (exchange === "FX_IDC") { setPrice(null); return; }

    const key = process.env.NEXT_PUBLIC_FINNHUB_API_KEY;
    if (!key) { setPrice(null); return; }

    let cancelled = false;
    setLoading(true);
    fetch(`https://finnhub.io/api/v1/quote?symbol=${symbol}&token=${key}`)
      .then(r => r.json())
      .then(d => { if (!cancelled && d?.c > 0) setPrice(Number(d.c)); else if (!cancelled) setPrice(null); })
  ...
  return { price: effectivePrice, autoPrice: price, manualPrice, setManualPrice, loading };
}
...
function TradingViewChart({ symbol, exchange, interval, studies }: {
  ...
  useEffect(() => {
    ...
    script.innerHTML = JSON.stringify({
      ...
      // ใช้สีเดียวกับพื้นหลังแทน transparent เพื่อให้ Grid หายแน่นอน
      ...
      // ล็อกไว้เฉพาะ RSI ค่ามาตรฐาน
      studies: ["RSI@tv-basicstudies"],

      // ใส่ซ้ำเป็น fallback สำหรับ Widget บางเวอร์ชัน
      overrides: {
        "paneProperties.background": "#0d0d0f",
        "paneProperties.backgroundType": "solid",
        "paneProperties.vertGridProperties.color": "#0d0d0f",
        "paneProperties.horzGridProperties.color": "#0d0d0f",
...
function SREditor({ symbol, currentPrice, manualPrice, setManualPrice, autoPrice }: {
  symbol: string;
    ...
    const load = async () => {
      ...
      try {
        ...
        if (user) {
          const { data } = await supabase.from("sr_levels").select("*")
            .eq("user_id", user.id).eq("ticker", symbol).maybeSingle();
          if (!cancelled && data) {
      ...
      try {
        ...
        if (raw && !cancelled) {
          ...
          setSupports([]); setResists([]); setInvest(0);
        }
      } catch {
        if (!cancelled) { setSupports([]); setResists([]); setInvest(0); }
      }
  ...
  const saveSR = async (nextSupports: number[], nextResists: number[]) => {
    ...
    try {
      localStorage.setItem(`sr_${symbol}`, JSON.stringify({
        s: nextSupports, r: nextResists, invest,
      }));
    } catch {}
  ...
  return (
    <div className="bg-[#18181b] border border-zinc-800 rounded-xl overflow-hidden">
      <div className="px-3 py-2.5 border-b border-zinc-800 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button onClick={() => setEditMode(!editMode)}
            className={`px-2 py-0.5 rounded text-[10px] font-bold transition-colors ${
              editMode ? "bg-yellow-400 text-black" : "bg-zinc-800 text-zinc-400"
            }`}>
      ...
      <div className="px-3 py-2 border-b border-zinc-800/60 bg-[#111113]">
        {showManualInput ? (
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-zinc-500 flex-shrink-0">ราคาตอนนี้</span>
      ...
      <div className="p-3 space-y-1">
        {loading && <p className="text-[10px] text-zinc-600 text-center py-2">โหลด…</p>}

        {!loading && (
          <>
            {/* Resistance */}
            <p className="text-[10px] text-zinc-500 uppercase tracking-wider mb-1">Resistance 🔴</p>
            {resists.length === 0 && <p className="text-[10px] text-zinc-700 mb-1.5">ยังไม่มี</p>}
            {resists.map((r, i) => {
              const d = fmtDist(r);
              return (
                <div key={i} className="flex items-center gap-2 px-2 py-1 bg-red-500/5 border border-red-500/20 rounded-lg">
                  <div className={`w-2 h-2 rounded-sm flex-shrink-0 ${typeof d === "object" && d.near ? "bg-red-400 animate-pulse" : "bg-red-400"}`}/>
                  <span className="text-[10px] text-zinc-500">R{i + 1}</span>
                  <span className="text-xs font-black font-mono text-red-400">{r.toFixed(3)}</span>
                  {typeof d === "object" && (
                    <span className={`text-[9px] font-mono ml-auto ${d.near ? "text-yellow-400 font-bold" : "text-zinc-600"}`}>
                      {d.text}{d.near && " ⚠"}
                    </span>
                  )}
                  {editMode && (
                    <button onClick={() => delResist(i)} className="text-zinc-600 hover:text-red-400 text-xs ml-1">✕</button>
                  )}
                </div>
              );
            })}
            {editMode && (
              <div className="flex gap-1.5 mt-1.5">
                <input type="number" value={newResist} onChange={e => setNewResist(e.target.value)}
                ...
                <button onClick={addResist} className="px-2.5 py-1 bg-red-500/20 border border-red-500/40 text-red-300 rounded-lg text-[11px] font-bold">+</button>
              </div>
            )}

            {/* Support */}
            <p className="text-[10px] text-zinc-500 uppercase tracking-wider mb-1 mt-3">Support 🟢</p>
            {supports.length === 0 && <p className="text-[10px] text-zinc-700 mb-1.5">ยังไม่มี</p>}
            {supports.map((s, i) => {
              const d = fmtDist(s);
              return (
                <div key={i} className="flex items-center gap-2 px-2 py-1 bg-emerald-500/5 border border-emerald-500/20 rounded-lg">
                  <div className={`w-2 h-2 rounded-sm flex-shrink-0 ${typeof d === "object" && d.near ? "bg-emerald-400 animate-pulse" : "bg-emerald-400"}`}/>
                  <span className="text-[10px] text-zinc-500">S{i + 1}</span>
                  <span className="text-xs font-black font-mono text-emerald-400">{s.toFixed(3)}</span>
                  {typeof d === "object" && (
                    <span className={`text-[9px] font-mono ml-auto ${d.near ? "text-yellow-400 font-bold" : "text-zinc-600"}`}>
                      {d.text}{d.near && " ⚠"}
                    </span>
                  ...
                  {editMode && (
                    <button onClick={() => delSupport(i)} className="text-zinc-600 hover:text-red-400 text-xs ml-1">✕</button>
                  )}
                </div>
              );
            })}
            {editMode && (
              <div className="flex gap-1.5 mt-1.5">
                <input type="number" value={newSupport} onChange={e => setNewSupport(e.target.value)}
                ...
                <button onClick={addSupport} className="px-2.5 py-1 bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 rounded-lg text-[11px] font-bold">+</button>
              </div>
            )}
...
function ChartPageInner() {
  ...
  const { price, autoPrice, manualPrice, setManualPrice } = useCurrentPrice(symbol, exchange);
  ...
  const sidePanel = (
    <>
      <FibCalculator/>
      <SREditor symbol={symbol} currentPrice={price} manualPrice={manualPrice}
        setManualPrice={setManualPrice} autoPrice={autoPrice}/>
    </>
  );
      ...
      <main className="h-screen bg-[#0d0d0f] text-white flex flex-col overflow-hidden select-none"
        ...
        <header className="flex-shrink-0 bg-[#0a0a0c] border-b border-zinc-800 px-3 py-2 flex items-center gap-2 z-20">
          ...
          <button onClick={() => setShowSymbolSheet(true)}
            ...
            {price && <span className="text-[10px] font-mono text-zinc-400">{price.toFixed(2)}</span>}
    ...
    <main className="h-screen bg-[#0d0d0f] text-white flex flex-col overflow-hidden"
      ...
      <header className="flex-shrink-0 border-b border-zinc-800 px-4 py-2.5 flex items-center gap-3 bg-[#0d0d0f]/95 backdrop-blur z-20">
        ...
        <button onClick={() => setShowSymbolSheet(!showSymbolSheet)}
          ...
          {price && <span className="text-xs font-mono text-zinc-400">{price.toFixed(2)}</span>}
