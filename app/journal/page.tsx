...
function PLChart({trades}:{trades:Trade[]}) {
  ...
  if(closed.length<2) return (
    <div style={{
      ...
      background:"linear-gradient(180deg,#080909,#040505)",
      border:"1px solid rgba(255,255,255,.09)",
      borderRadius:5,color:"#686d6d",
  ...
  // Compact responsive SVG coordinate system.
  const W=1000,H=330,pL=18,pR=48,pT=18,pB=40;
  const iW=W-pL-pR,iH=H-pT-pB;
  const mn=Math.min(0,...pts),mx=Math.max(0,...pts),rng=Math.max(1,mx-mn);
  const X=(i:number)=>pL+(n===1?iW/2:(i/(n-1))*iW);
  const Y=(v:number)=>pT+iH-((v-mn)/rng)*iH;

  const {dd,maxDD:mxDD,ddPct,maxDDPct}=calcDD(trades);
  let peak=0;
  const dds=pts.map(v=>{peak=Math.max(peak,v);return Math.max(0,peak-v);});
  const ddMax=Math.max(1,...dds);
  const Yd=(v:number)=>pT+(v/ddMax)*iH;

  let equityPath=`M ${X(0)} ${Y(pts[0])}`;
  for(let i=1;i<n;i++){
    equityPath+=` L ${X(i)} ${Y(pts[i-1])} L ${X(i)} ${Y(pts[i])}`;
  }
  const equityArea=`${equityPath} L ${X(n-1)} ${pT+iH} L ${X(0)} ${pT+iH} Z`;

  let ddPath=`M ${X(0)} ${Yd(dds[0])}`;
  for(let i=1;i<n;i++){
    ddPath+=` L ${X(i)} ${Yd(dds[i-1])} L ${X(i)} ${Yd(dds[i])}`;
  }
  const ddArea=`${ddPath} L ${X(n-1)} ${pT+iH} L ${X(0)} ${pT+iH} Z`;
  ...
  const tabStyle=(active:boolean):React.CSSProperties=>({
    ...
    borderBottom:active?"1px solid #080909":"1px solid rgba(255,255,255,.12)",
    borderRadius:"5px 5px 0 0",
    marginBottom:active?-1:0,
    background:active?"#0b0c0c":"#060707",
    color:active?"#f0f0ed":"#747878",
  ...
  return (
    <div className="j-equity-chart" style={{
      background:"linear-gradient(145deg,#090a0a,#050606)",
      ...
      <div style={{
        ...
        <div style={{display:"flex",gap:4,height:"100%",alignItems:"end"}}>
          <button type="button" onClick={()=>setTab("equity")} style={tabStyle(tab==="equity")}>▰ &nbsp;Equity</button>
          <button type="button" onClick={()=>setTab("dd")} style={tabStyle(tab==="dd")}>▱ &nbsp;Drawdown</button>
        </div>
        <div style={{
          fontFamily:"'DM Mono',monospace",fontSize:9,
          color:"#686d6d",letterSpacing:".10em"
        }}>
          {n} TRADES
        </div>
      </div>
      ...
      <div style={{padding:"10px 10px 8px"}}>
        <div style={{
          ...
          {tab==="equity"?(
            <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="auto"
              ...
              {pts.map((_,i)=>(
                <line key={`v${i}`}
                  x1={X(i)} y1={pT} x2={X(i)} y2={pT+iH}
                  stroke="rgba(255,255,255,.055)" strokeWidth="1"
                  strokeDasharray={i%labelEvery===0?"3 5":"1 7"}
                />
              ))}
              {/* Horizontal grid */}
              {ticks.map((r,i)=>(
                <line key={`h${i}`}
                  x1={pL} y1={pT+iH*r} x2={W-pR} y2={pT+iH*r}
                  stroke="rgba(255,255,255,.065)" strokeWidth="1"
                />
              ))}
              <line x1={pL} y1={Y(0)} x2={W-pR} y2={Y(0)}
                ...
                strokeDasharray="5 5"/>
              <defs>
                <linearGradient id="eqFillNinja" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#d92332" stopOpacity=".38"/>
                  <stop offset="100%" stopColor="#d92332" stopOpacity=".025"/>
                </linearGradient>
              </defs>
              <path d={equityArea} fill="url(#eqFillNinja)"/>
              <path d={equityPath}
                fill="none" stroke="#e11d2e" strokeWidth="2.2"
                strokeLinecap="square" strokeLinejoin="miter"/>
              {pts.map((v,i)=>(
                <rect key={`p${i}`}
                  x={X(i)-3.2} y={Y(v)-3.2} width="6.4" height="6.4"
                  fill="#080909" stroke="#e11d2e" strokeWidth="1.5"
                />
              ))}
              {/* X axis labels */}
              {pts.map((_,i)=>(
                (i===0 || i===n-1 || i%labelEvery===0) &&
                <text key={`xl${i}`} x={X(i)} y={H-14}
                  textAnchor="middle" fill="#656a6a"
                  fontFamily="'DM Mono',monospace" fontSize="9">
                  {i+1}
                </text>
              ))}
              <text x={W/2} y={H-3} textAnchor="middle"
                fill="#666b6b" fontFamily="'DM Mono',monospace" fontSize="8"
                letterSpacing="2">TRADES</text>
            </svg>
          ):(
            <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="auto"
              preserveAspectRatio="none" role="img" aria-label="Drawdown curve">
              {dds.map((_,i)=>(
                <line key={`v${i}`}
                  x1={X(i)} y1={pT} x2={X(i)} y2={pT+iH}
                  stroke="rgba(255,255,255,.055)" strokeWidth="1"
              ...
              <defs>
                <linearGradient id="ddFillNinja" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#8e111c" stopOpacity=".40"/>
                  <stop offset="100%" stopColor="#8e111c" stopOpacity=".04"/>
                </linearGradient>
              </defs>
              <path d={ddArea} fill="url(#ddFillNinja)"/>
              <path d={ddPath}
                fill="none" stroke="#d92332" strokeWidth="2.2"
                strokeLinecap="square" strokeLinejoin="miter"/>
              {dds.map((v,i)=>(
                <rect key={`p${i}`}
                  x={X(i)-3.2} y={Yd(v)-3.2} width="6.4" height="6.4"
                  fill="#080909" stroke="#d92332" strokeWidth="1.5"
                />
              ))}
              {dds.map((_,i)=>(
                (i===0 || i===n-1 || i%labelEvery===0) &&
        ...
        {tab==="dd"&&(
          <div style={{
            ...
            {[
              {l:"CURRENT DD",v:dd>0?`-$${dd.toFixed(2)}`:"+$0.00",c:dd>0?"#d92332":"#6caa89",s:ddPct>0?`-${ddPct.toFixed(1)}%`:"0%"},
              {l:"MAX DD",v:mxDD>0?`-$${mxDD.toFixed(2)}`:"+$0.00",c:"#d92332",s:maxDDPct>0?`-${maxDDPct.toFixed(1)}%`:"0%"},
              {l:"STATUS",v:maxDDPct<=10?"SAFE":maxDDPct<=20?"WATCH":"DANGER",c:maxDDPct>20?"#d92332":maxDDPct>10?"#c6a45b":"#6caa89",s:"LIMIT 20%"}
            ].map(s=>(
              <div key={s.l} style={{
                background:"#090a0a",border:"1px solid rgba(255,255,255,.08)",
                borderRadius:4,padding:"8px"
              }}>
                <div style={{fontFamily:"'DM Mono',monospace",fontSize:7.5,color:"#626767",letterSpacing:".09em"}}>{s.l}</div>
                <div style={{fontFamily:"'VT323',monospace",fontSize:20,color:s.c,lineHeight:1.1,marginTop:3}}>{s.v}</div>
                <div style={{fontFamily:"'DM Mono',monospace",fontSize:7.5,color:"#777b7b",marginTop:2}}>{s.s}</div>
              </div>
            ))}
          </div>
        )}
