...
วิเคราะห์ตามระบบ SMC Pro Max แล้วตอบ JSON นี้เท่านั้น:
{
  ...
  "reasons": ["r1", "r2", "r3"]
    ...
    // ── Hard Gate Override — บังคับ verdict ที่ backend กัน AI บอกเข้าทั้งที่ยังไม่ครบ ──
    const applyHardGate = (parsed: any) => {
      const c = parsed.smcChecklist || {};
      const hardPassed =
        c.step5_swept &&      // $$$ ถูก sweep แล้ว
        c.step6_rejection &&  // มี Rejection
        c.step7_ltfRetest &&  // LTF Retest ถูกเงื่อนไข
        c.step8_mss &&        // MSS ผ่าน
        c.step10_m15Retest && // M15 Retest แล้ว
        c.step11_sl_ready;    // SL พร้อม

      const score = Number(parsed.setupScore || 0);

      // Non-SMC setup ที่ active อยู่ ไม่ต้องผ่าน SMC Hard Gate (ใช้ backup setup ได้)
      const nonSmcActive = parsed.nonSmcSetup?.active === true;

      if (lossesToday >= 3) {
        // แพ้ 3 ไม้ = Daily Lock บังคับหยุด
        parsed.verdict = "ไม่เข้า";
        parsed.actionPlan = { ...parsed.actionPlan, direction: "รอ" };
        parsed.hardGateFail = `🚫 Daily Lock: แพ้ครบ ${lossesToday} ไม้แล้ว วันนี้หยุดเทรด`;
      } else if (parsed.setupGroup === "SMC" && (!hardPassed || score < 80)) {
        // ถ้าเลือก SMC แต่ Hard Gate ไม่ครบ หรือ score ต่ำ → บังคับรอ/ไม่เข้า
        parsed.verdict = score >= 50 ? "รอ" : "ไม่เข้า";
        parsed.actionPlan = { ...parsed.actionPlan, direction: "รอ" };
        parsed.hardGateFail = parsed.hardGateFail && parsed.hardGateFail !== "null"
          ? parsed.hardGateFail
          : "Hard Gate ไม่ครบ หรือ Setup Score ต่ำกว่า 80";
      } else if (!nonSmcActive && parsed.setupGroup !== "SMC" && parsed.verdict === "เข้า") {
        // ไม่ใช่ SMC และไม่มี Non-SMC setup ที่ active → ไม่ให้เข้า
        parsed.verdict = "รอ";
        parsed.actionPlan = { ...parsed.actionPlan, direction: "รอ" };
        parsed.hardGateFail = parsed.hardGateFail || "ไม่มี Setup ที่ครบเงื่อนไขให้เข้า";
      }
    ...
    try {
      ...
      return NextResponse.json(applyHardGate(JSON.parse(cleaned)));
    } catch {}
    try {
      const match = text.match(/\{[\s\S]*\}/);
      if (match) return NextResponse.json(applyHardGate(JSON.parse(match[0])));
    } catch {}
