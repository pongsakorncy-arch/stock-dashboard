...
async function uploadImage(supabase: ReturnType<typeof adminClient>, dataUrl: string, side: "htf" | "ltf", userId?: string | null) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { userId, htfImage, ltfImage, aiResult, market = "XAUUSD", timeframe = "HTF+LTF", version = "claude-ui-v1" } = body;

    if (!htfImage || !ltfImage || !aiResult) {
      return NextResponse.json({ error: "Missing htfImage, ltfImage, or aiResult" }, { status: 400 });
    }
