...
export default function LoginPage() {
  ...
  const loginWithGitHub = async () => {
    ...
    // ดึงค่า URL หลัก (Origin) ของแอป ณ ขณะนั้นแบบ Dynamic
    const origin = window.location.origin;

    await supabase.auth.signInWithOAuth({
      provider: "github",
      options: {
        // จะกลายเป็น http://localhost:3000/auth/callback หรือ https://... ตามที่ผู้ใช้เปิดอยู่จริง
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0c] flex items-center justify-center" style={{ fontFamily: "'Inter','Noto Sans Thai',sans-serif" }}>
