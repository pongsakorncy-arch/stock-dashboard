...
function HardLockBanner({onWriteReflection,submitted}:{onWriteReflection:()=>void;submitted:boolean}) {
  return (
    <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
      <div style={{padding:"9px 14px",background:"var(--j-coral)",border:"2.5px solid var(--j-ink)",borderRadius:9,fontSize:12,fontFamily:"'Fredoka',sans-serif",fontWeight:700,boxShadow:"3px 3px 0 var(--j-ink)",lineHeight:1.35}}>
        🛑 วันนี้แพ้ครบ 3 ไม้แล้ว — พักเทรดไปเลย
      </div>
      {!submitted && (
...
export default function JournalPage() {
  ...
  // Hard lock ต้องอิงจากข้อมูล Session ปัจจุบันเท่านั้น
  // ถ้าลบ/แก้ Session แล้ว Loss streak ไม่ถึง 3 ไม้ Banner ต้องหายทันที
  const isHardLockToday = dailyStatus.isHardStop;
  ...
  // ── Reconcile discipline lock after edit/delete sessions ─────────────────────
  // สำคัญ: ถ้าลบไม้ทดสอบออกแล้ว จำนวน Loss วันนี้ไม่ถึงเงื่อนไข ระบบต้องปลด Banner เอง
  useEffect(()=>{
    // Hard lock เก็บไว้เพื่อ reflection เท่านั้น ไม่ควรเป็นตัวล็อกหลักถ้าข้อมูล Session เปลี่ยนแล้ว
    if (!dailyStatus.isHardStop && hardlock?.date === todayStr) {
      saveHardlock(null);
      setHardlock(null);
      setShowReflection(false);
      try { localStorage.removeItem(ALERT_ACK_KEY); } catch {}
    }

    // Cooldown ควรอยู่เฉพาะตอน Loss streak = 2 และยังไม่ Hard stop
    if (dailyStatus.lossStreak < 2 || dailyStatus.isHardStop) {
      if (cooldownUntil > 0) {
        saveCooldownUntil(0);
        setCooldownUntil(0);
      }
    }
  }, [dailyStatus.isHardStop, dailyStatus.lossStreak, hardlock?.date, todayStr, cooldownUntil]);
              ...
              <Win title="🔒 ล็อกอยู่ — เข้าไม้ไม่ได้ตอนนี้" color="var(--j-coral)">
                <div style={{textAlign:"center",padding:"12px 4px"}}>
                  ...
                  <div style={{fontFamily:"'Fredoka',sans-serif",fontSize:15,fontWeight:700,marginBottom:10}}>
                    {isForcedLockToday?"วันนี้ล็อกทั้งวัน (Pattern ซ้ำในสัปดาห์)":isHardLockToday?"วันนี้แพ้ครบ 3 ไม้แล้ว — พักเทรดไปเลย":`พักบังคับ เหลือ ${fmtMMSS(cooldownRemainingMs)}`}
                  </div>
          ...
          <div className="j-win" style={{maxWidth:340,width:"100%"}}>
            ...
            <div className="j-body" style={{textAlign:"center",padding:"24px 20px"}}>
              ...
              <div style={{fontFamily:"'Fredoka',sans-serif",fontSize:18,fontWeight:700,marginBottom:8}}>
                {dailyStatus.isHardStop?"วันนี้พอแล้ว พักเทรดไปเลย 🌿":"วันนี้ทำหน้าที่ครบแล้ว 👏"}
              </div>
              <div style={{fontFamily:"'DM Mono',monospace",fontSize:11,color:"var(--j-soft)",marginBottom:20,lineHeight:1.8}}>
                {dailyStatus.isHardStop?`"ตลาดไม่ไปไหน พรุ่งนี้ค่อยกลับมาใหม่"`:`"พักผ่อน review journal ได้เลย"`}
              </div>
